...
if file:
    ...
    if st.button("🚀 Proses"):
        ...
        # ===================== CLEAN TEXT =====================
        st.subheader("🧹 Hasil Clean Text")
        st.dataframe(df[[text_col, "clean_text"]].head(20))
        ...
        # ===================== STATISTIK TF-IDF =====================
        st.subheader("📊 Statistik Hasil Analisis TF-IDF")

        total_kata = len(tfidf_df)
        tfidf_max = tfidf_df["Skor_TFIDF"].max()
        tfidf_min = tfidf_df["Skor_TFIDF"].min()
        tfidf_avg = tfidf_df["Skor_TFIDF"].mean()

        st.markdown(f"""
        ✅ **Hasil TF-IDF disimpan ke:** `tfidf_scores_results.csv`

        📊 **STATISTIK HASIL ANALISIS:**
        - Total kata yang dianalisis: **{total_kata}**
        - Skor TF-IDF tertinggi: **{tfidf_max:.4f}**
        - Skor TF-IDF terendah: **{tfidf_min:.4f}**
        - Skor TF-IDF rata-rata: **{tfidf_avg:.4f}**
        """)

        tfidf_df.to_csv("tfidf_scores_results.csv", index=False)

        st.download_button(
            "⬇️ Download TF-IDF",
            tfidf_df.to_csv(index=False),
            "tfidf_scores_results.csv",
            "text/csv"
        )
